...
ROOT=Path(__file__).resolve().parents[1]; PHASE=ROOT/'outputs'/'m8_m12'; REPORTS=ROOT/'outputs'/'reports'; FIG=ROOT/'outputs'/'figures'
...
def main():
    ...
    primary=pd.read_csv(ROOT/'outputs'/'primary_benchmark_m37'/'primary_comparison.csv'); comp=load(ROOT/'outputs'/'primary_benchmark_m37'/'transformer_comparison.json'); ab=pd.read_csv(PHASE/'m8_feature_ablations.csv'); ood=pd.read_csv(PHASE/'m9_speed_ood.csv'); phase=pd.read_csv(PHASE/'m11_phase_lag.csv'); m10=load(PHASE/'m10_multitask_summary.json');make_figures(primary,ood,phase)
