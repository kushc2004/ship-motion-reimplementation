...
ROOT=Path(__file__).resolve().parents[1];PHASE=ROOT/'outputs'/'m8_m12';OUT=ROOT/'outputs'/'cv'
def main():
    ...
    comp=json.loads((ROOT/'outputs'/'primary_benchmark_m37'/'transformer_comparison.json').read_text());m10=json.loads(files[1].read_text());multi=next(r for r in m10 if r['task']=='multitask' and r['threshold_m']==.5)
